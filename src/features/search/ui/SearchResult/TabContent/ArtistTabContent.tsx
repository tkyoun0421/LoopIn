...
const ArtistTabContent = (): JSX.Element => {
  ...
  const aggregatedArtistData = useMemo(() => {
    ...
    if (!firstPage?.artists) return undefined;

    const allArtists = typedInfiniteData.pages.flatMap(
      (page: SearchForItemResponse) => page.artists?.items || [],
    );
  ...
  if (isLoading) {
    return <SearchResultSkeleton type="artist" count={INFINITE_QUERY_LIMIT} />;
  }

  return (
    <div className="space-y-6">
      <SearchResultArtist artists={aggregatedArtistData} isLoading={false} />
      {hasNextPage && (
